	...
	?>
		...
		<?php echo __( 'Crawler Cron', 'litespeed-cache' ); ?>
			<span class="litespeed-switch-drag litespeed-cron-onoff-btn">
				...
				<?php
				if ( $_options[ LiteSpeed_Cache_Config::CRWL_CRON_ACTIVE ] && LiteSpeed_Cache_Router::can_crawl() ) {
					echo 'checked';}
				?>
				<?php echo $disabled; ?>
				/>
		...
		<div class="litespeed-desc">
			...
			<div>
			<?php
			echo __( 'Only one crawler can run concurrently.', 'litespeed-cache' )
				. __( 'If both the cron and manual run start at a similar time, the first one to start will run.', 'litespeed-cache' );
			?>
					</div>
