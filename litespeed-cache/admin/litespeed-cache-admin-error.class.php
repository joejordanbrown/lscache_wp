	...
	public function convert_code_to_error( $err_code ) {
		...
		switch ( $err_code ) {
			case self::E_PHP_VER:
				...
				. sprintf(
					wp_kses(
						__( 'Please upgrade or go to <a href="%s">active plugins</a> and deactivate the LiteSpeed Cache plugin to hide this message.', 'litespeed-cache' ),
			...
			case self::E_SETTING_CAT:
				...
				return __(
					'Removed category "%s" from list, ID does not exist.',
					'litespeed-cache'
				);
			...
			case self::E_SETTING_TAG:
				...
				return __(
					'Removed tag "%s" from list, ID does not exist.',
					'litespeed-cache'
				);

			case self::E_SETTING_LC:
				return __(
					'Invalid login cookie. Invalid characters found: %s',
					'litespeed-cache'
				);
