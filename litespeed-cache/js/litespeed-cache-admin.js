...
(function ($) {
	...
	jQuery( document ).ready(
		function () {
			...
			$( '[name="litespeed-cache-conf[mobileview_enabled]"][value=1]' ).click(
				function() {
					if ($( this ).is( ':checked' )) {
			...
			$( '[name="litespeed-cache-conf[mobileview_enabled]"][value=0]' ).click(
				function() {
					if ($( this ).is( ':checked' )) {
			...
			$( '#litespeed_manual_trigger' ).click(
				function(event) {
			...
			$( '#litespeed_crawler_cron_enable' ).click(
				function(event) {
					var that = this;
					$.getJSON(
						$( that ).data( 'url' ),
			...
			$( document ).on(
				'click',
				'.lscwp-whm-notice .notice-dismiss',
				function () {
			...
			$( document ).on(
				'click',
				'.lscwp-notice-ruleconflict .notice-dismiss',
				function () {
				...
				function() {
					...
					$( document ).on(
						...
						function( e ) {
							...
							if ( litespeed_keycode( thiskey.charCodeAt( 0 ) ) ) {
								$( that )[ 0 ].click();
							}
						}
					);
			...
			/** CDN mapping **/
			$( '#litespeed-cdn-mapping-add' ).click(
				function(event) {
			...
			function litespeed_cdn_mapping_del_init() {
				$( '[data-litespeed-cdn-mapping-del]' ).off().click(
					function(event) {
						if ( ! confirm( 'Are you sure?' ) ) {
							return false;
						}
						$( this ).closest( '[data-litespeed-cdn-mapping]' ).fadeOut(
							400,
							function(){
								$( this ).remove();
							}
			...
			function litespeed_toggle() {
				$( '[data-litespeed-toggle-on]' ).off().click(
					function(event) {
...
function litespeed_display_interval_reset() {
	...
	jQuery( '.litespeed-shell-header-bar' ).stop().animate(
		{width: '100%'},
		500,
		function(){
