...
class LiteSpeed_Cache_Config extends LiteSpeed_Cache_Const {
	...
	private function construct_multisite_options() {
		...
		if ( ! empty( $site_options[ self::NETWORK_OPID_USE_PRIMARY ] ) ) {
			...
			$options = get_blog_option( BLOG_ID_CURRENT_SITE, self::OPTION_NAME, array() );
	...
	public function plugin_upgrade() {
		...
		if ( isset( $this->options['cdn_url'] ) ) {
			...
			update_option( self::ITEM_CDN_MAPPING, array( $cdn_mapping ) );
		...
		if ( isset( $this->options['forced_cache_uri'] ) ) {
			update_option( self::ITEM_FORCE_CACHE_URI, $this->options['forced_cache_uri'] );
		}
		if ( isset( $this->options['cache_uri_priv'] ) ) {
			update_option( self::ITEM_CACHE_URI_PRIV, $this->options['cache_uri_priv'] );
		}
		if ( isset( $this->options['optm_excludes'] ) ) {
			update_option( self::ITEM_OPTM_EXCLUDES, $this->options['optm_excludes'] );
		}
		if ( isset( $this->options['excludes_uri'] ) ) {
			update_option( self::ITEM_EXCLUDES_URI, $this->options['excludes_uri'] );
		}
