...
class LiteSpeed_Cache_Img_Optm {
	...
	private function _request_optm() {
		...
		foreach ( $list as $v ) {
			...
			if ( $this->_img_total + $num_will_incease > $_credit ) {
				if ( ! $this->_img_total ) {
					$msg = sprintf( __( 'Number of images in one image group (%1$s) exceeds the credit (%2$s)', 'litespeed-cache' ), $num_will_incease, $_credit );
	...
	private function _img_optimize_rescan() {
		...
		foreach ( $list as $v ) {
			...
			$optm_data_pid2mid_list[ $v->post_id ] = array(
				'status_mid' => $v->bmeta_id,
				'data_mid'   => $v->cmeta_id,
			);
		...
		switch ( $type ) {
			...
			case self::TYPE_IMG_PULL:
				...
				if ( is_array( $result ) && $result['ok'] === 'to_be_continued' ) {
					$link = LiteSpeed_Cache_Utility::build_url( LiteSpeed_Cache::ACTION_IMG_OPTM, self::TYPE_IMG_PULL );
